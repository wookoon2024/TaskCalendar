...
* **[신규] 음력 기준 매년 반복 일정 (생일/제사/차례)**:
  - 제사, 부모님 생신 등 음력 기준 일정을 등록할 때 **[반복] ➔ [매년 (음력)]**을 선택하면, 선택한 날짜에 해당하는 **음력 날짜(예: 🌙 음력 8월 15일)가 자동으로 계산 및 안내**되며 매년 정확한 양력 일자에 일정이 자동 표시됩니다.
* **[신규] 빠른 기간 설정 버튼 (1일 ~ 영구)**:
  - 일시 입력란 바로 아랫줄에 **`[1일]` `[1주일]` `[1달]` `[3개월]` `[6개월]` `[1년]` `[10년]` `[영구]`** 버튼을 제공합니다.
  - 달력을 번거롭게 넘길 필요 없이 버튼 클릭 한 번으로 종료일을 즉시 자동 완성할 수 있습니다.
* **[신규] 다채로운 스티커 & 사용자 PNG 스티커 직접 등록/삭제**:
  - `[스티커...]` 버튼을 누르면 직관적인 비주얼 팝업이 열립니다.
  - **내 스티커**: 사용자가 소장한 투명 PNG 이미지 파일을 직접 등록(`➕ PNG 스티커 등록...`)하여 나만의 맞춤 스티커로 꾸밀 수 있으며, 필요 없는 스티커는 마우스 우클릭으로 간편하게 삭제할 수 있습니다.
  - **기본 스티커 & 카테고리별 이모지**: 기본 내장 스티커와 업무, 기념일, 일상, 강조 등 다양한 아이콘을 깔끔한 탭별로 손쉽게 선택할 수 있습니다.
* **반복 일정 지원**: 매일, 매주, 매월 특정 요일/날짜, 매년 (양력/음력) 등 다양한 반복 패턴을 지원합니다.
...
![음력 반복 및 스티커 등록](./08_lunar_recurrence_dialog.png)
![스티커 피커 팝업](./07_sticker_picker.png)

---

## 🌙 4. 음력 및 우리나라 24절기 자동 표기

![음력 및 24절기 캘린더 화면](./06_lunar_and_solar_terms.png)

* **24절기 자동 강조 표기**:
  - 입춘, 우수, 경칩, 춘분, 하지, 백로, 추분, 동지 등 24절기 당일에 눈에 띄는 산뜻한 녹색으로 절기 명칭이 표시됩니다.
* **은은한 음력 날짜 표시**:
  - 각 날짜 셀 상단에 음력 일자(예: `7.29`, `8.15`)가 은은하게 표시되어 제사나 기념일 확인 시 별도의 달력을 찾을 필요가 없습니다.
* **상단 월 이동 버튼 가독성 강화**:
  - 이전 달(`◀`), 다음 달(`▶`) 이동 버튼을 크고 선명한 디자인으로 개선하여 월 전환 조작이 더욱 편리해졌습니다.
* **독립적인 On/Off 설정 제공**:
  - 환경설정 [기본] ➔ **"캘린더에 음력 날짜 표시"** 및 **"캘린더에 24절기 표시"** 두 옵션을 각각 독립적으로 켜거나 끌 수 있습니다.
